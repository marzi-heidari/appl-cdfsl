...
class SimpleDataset:
    ...
    def __getitem__(self, i):
        names_ = self.meta['image_names']
        img = self.transform(names_[i])
        img2 = self.transform(names_[i])
        img3 = self.transform(names_[i])
        img4 = self.transform(names_[i])
        img5 = self.transform(names_[i])
        target = self.target_transform(self.meta['image_labels'][i])
        return img, img2, img3, img4, img5, target
...
class SetDataset:
    def __init__(self, batch_size, transform_orig, transform_aug):
        ...
        sub_data_loader_params = dict(batch_size=batch_size,
                                      ...
                                      pin_memory=False)
        for cl in self.cl_list:
            sub_dataset = SubDataset(self.sub_meta[cl], cl, transform_orig=transform_orig, transform_aug=transform_aug)
...
class SubDataset:
    def __init__(self, sub_meta, cl, transform_orig=transforms.ToTensor(), transform_aug=transforms.ToTensor(),
    ...
    def __getitem__(self, i):
        img = self.transform_orig(self.sub_meta[i])
...
class SetDataManager(DataManager):
    ...
    def get_data_loader(self, aug):  # parameters that would change on train/val set
        transform_orig = self.trans_loader.get_composed_transform(False)
        transform_aug = self.trans_loader.get_composed_transform(TransformLoader)
        dataset = SetDataset(self.batch_size, transform_orig, transform_aug)
