...
class SimpleDataset:
    def __init__(self, transform, target_transform=identity):
        ...
        for i, (data, label) in enumerate(d):
            self.meta['image_names'].append(transform(data))
            self.meta['image_labels'].append(target_transform(label))
...
class SetDataset:
    def __init__(self, batch_size, transform):
        ...
        for i, (data, label) in tqdm(enumerate(d)):
            # transformed_image = transform(data)
            # transformed_image2 = transform(data)
            # U, s, V = np.linalg.svd(transformed_image)
            # dim = 224 // 3
            # S = np.zeros(np.shape(transformed_image))
            # for k in range(2):
            #     for j in range(0, dim):
            #         S[k][j, j] = s[k][j]
            # s_v = U @ S @ V
            # self.sub_meta[label].append(torch.from_numpy(s_v).float())
