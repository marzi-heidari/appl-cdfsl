...
def distributed_wrn28_10(num_classes=200, loss_type='dist'):
    model = DistributedWideResNet(depth=28, widen_factor=10, num_classes=num_classes, loss_type=loss_type,
                                  per_img_std=False,
                                  stride=1)
    return model


class DistributedWideResNet(WideResNet):
    def __init__(self, depth=28, widen_factor=10, num_classes=200, loss_type='dist', per_img_std=False, stride=1):
        super().__init__(depth, widen_factor, num_classes, loss_type, per_img_std, stride)
        self.conv1 = self.conv1.cuda(1)
        self.block1 = self.block1.cuda(2)
        # self.conv1 = self.conv1.cuda(0)
        # self.block1 = self.block1.cuda(0)
        self.block2 = self.block2.cuda(3)
        self.block3 = self.block3.cuda(0)
        self.bn1 = self.bn1.cuda(0)
        self.relu = self.relu.cuda(0)
        self.linear = self.linear.cuda(0)

    def forward(self, x, target=None, mixup=False, mixup_hidden=True, mixup_alpha=None, lam=0.4):
        out = x
        out = self.conv1(out.cuda(1))
        out = self.block1(out.cuda(2))
        out = self.block2(out.cuda(3))
        out = self.block3(out.cuda(0))
        out = self.relu(self.bn1(out))
        out = F.avg_pool2d(out, out.size()[2:])
        out = out.view(out.size(0), -1)
        out1 = self.linear(out)
